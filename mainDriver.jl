...
gamma = 0.5 .* ones(N,J)     # each row is supply sector, and each column is demand sector, so column sum is 1
# each row is k, each col is j, gamma 12 means sector 1 use intermediates from sector 2
...
end     # sector 2 has higher trade cost than sector 1

# value added share, nj 
beta = 0.5 .* ones(N,J)     # implies that cty 1 production rely on intermediate more than labor
...
P0 = 0.5 .* ones(N, J)      # price is differ across ctys and sectors
